...
const HANZO_CLI_COMMAND = "hanzo-ui@latest"

export async function npxHanzo(command: string) {
  const packageRunner = await getPackageRunner(process.cwd())
  return `${packageRunner} ${HANZO_CLI_COMMAND} ${command}`
}
...
export function formatSearchResultsWithPagination(
  ...
  const formattedItems = results.items.map((item) => {
    ...
    parts.push(
      `\n  Add command: \`${npxHanzo(`add ${item.addCommandArgument}`)}\``
    )
