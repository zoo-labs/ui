...
export const headlessUIConfig: ScraperConfig = {
  name: 'headless-ui',
  ...
  transformers: {
    componentName: (name: string) => name.split('-').map((w, i) => i === 0 ? w : w.charAt(0).toUpperCase() + w.slice(1)).join(''),
    fileName: (name: string, framework: string) => `${name}.${framework === 'vue' ? 'vue' : 'tsx'}`,
    importPath: (name: string, framework: string) => `@headlessui/${framework}`,
    convertToFramework: (code: string, framework: 'react' | 'vue') => code
  },
