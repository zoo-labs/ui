...
Run the `hanzo-ui` init command to setup your project:
...
When running `npx @hanzo/ui@latest init`, your tailwind config for content will be overwritten. To fix this, change the `module.exports` to `export default` and the `content` section with the code below to your `tailwind.config.mjs` file:
