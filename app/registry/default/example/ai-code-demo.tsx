...
import { AICode, type CodeSuggestion, type Explanation, type SecurityIssue } from "@/registry/default/ui/ai-code"
...
function generatedFunction() {
  ...
  const handleExplain = async (code: string, line?: number): Promise<Explanation[]> => {
    ...
    lines.forEach((lineContent, index) => {
      ...
      if (lineContent.includes('export')) {
        explanations.push({
          id: `explain-${lineNumber}`,
          line: lineNumber,
          content: 'Export statement - makes this function/variable available to other modules',
          type: 'explanation'
        })
      }
      ...
      if (lineContent.includes('// TODO')) {
        explanations.push({
          id: `explain-${lineNumber}`,
          line: lineNumber,
          content: 'TODO comment - indicates functionality that needs to be implemented',
          type: 'warning'
        })
      }
    })
...
${refactoredCode}`
  ...
  const handleDebug = async (code: string): Promise<CodeSuggestion[]> => {
    ...
    if (code.includes('console.log')) {
      suggestions.push({
        ...
        description: 'Console.log statements found - consider using a proper logging library',
        severity: 'warning',
    ...
    if (!code.includes('error') && !code.includes('try')) {
      suggestions.push({
        ...
        description: 'No error handling detected - consider adding try-catch blocks',
        severity: 'warning'
      })
    }
  ...
  const handleTest = async (code: string): Promise<string> => {
    ...
    const functionName = code.match(/function\s+(\w+)/)?.[1] || 'generatedFunction'
...
- TypeScript documentation`
  ...
  const handleReview = async (code: string): Promise<CodeSuggestion[]> => {
    ...
    suggestions.push({
      ...
      type: 'review',
      title: 'Code Quality',
      description: 'Overall code structure looks good, but consider these improvements',
      severity: 'info'
    })
    ...
    if (!code.includes('export')) {
      suggestions.push({
        ...
        type: 'review',
        title: 'Add Exports',
        description: 'Consider adding export statements to make functions reusable',
        severity: 'warning'
      })
    }
  ...
  const handleSecurityScan = async (code: string): Promise<SecurityIssue[]> => {
    ...
    if (code.includes('innerHTML')) {
      issues.push({
        ...
        severity: 'high',
        message: 'Direct innerHTML manipulation detected - potential XSS vulnerability',
    ...
    if (code.includes('document.write')) {
      issues.push({
        ...
        severity: 'medium',
        message: 'Use of document.write() detected - can lead to XSS vulnerabilities',
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Try the AI-powered code assistant with features like code generation,
            explanation, refactoring, debugging, testing, and security scanning.
          </p>
        </div>
