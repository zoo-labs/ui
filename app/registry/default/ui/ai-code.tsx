...
import {
  Bot, User, Copy, Check, Play, Bug, TestTube, FileText,
  RotateCcw, Send, Wand2, Code, AlertTriangle, Shield,
  ChevronDown, ChevronRight, Eye, EyeOff, Loader2,
  Split, MessageSquare, Settings, Zap, FileCode,
  GitCompare, Download, Upload
} from "lucide-react"
...
>(({ issues, onFixIssue, className, ...props }, ref) => (
  <div ref={ref} className={cn("space-y-2", className)} {...props}>
    <div className="flex items-center gap-2">
      ...
      {issues.length > 0 && (
        <Badge variant={issues.some(i => i.severity === 'critical') ? 'destructive' : 'secondary'}>
        ...
        {issues.map((issue) => (
          <div
            ...
            className={cn(
              ...
              issue.severity === 'medium' && "border-yellow-500 bg-yellow-50 dark:bg-yellow-950",
              issue.severity === 'low' && "border-blue-500 bg-blue-50 dark:bg-blue-950"
            )}
          >
...
>(({ messages, onSendMessage, isGenerating, placeholder = "Ask AI about your code...", className, ...props }, ref) => {
...
>(({ files, activeFile, onFileChange, onFileCreate, onFileDelete, className, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("flex items-center gap-1 border-b bg-muted/30 px-2", className)} {...props}>
...
const AICode = React.forwardRef<HTMLDivElement, AICodeProps>(
  ({
    ...
    onSecurityScan,
    onChange,
    features = ['generate', 'explain', 'refactor', 'debug', 'test', 'document', 'review'],
    showMinimap = false,
    ...
    const [currentActiveFile, setCurrentActiveFile] = React.useState(activeFile || files[0]?.name)
    const [showSidePanel, setShowSidePanel] = React.useState(true)
    const [sidePanelTab, setSidePanelTab] = React.useState<'chat' | 'suggestions' | 'security'>('chat')
    ...
    React.useEffect(() => {
      if (currentFile) {
        ...
        setCurrentLanguage(currentFile.language || getLanguageFromFile(currentFile.name))
      }
    }, [currentFile])
    ...
    const handleEditorChange: OnChange = React.useCallback((newValue) => {
      ...
      monaco.languages.registerCodeActionProvider(currentLanguage, {
        provideCodeActions: (model, range) => {
          const actions = suggestions
            .filter(s => s.startLine && s.endLine &&
              range.startLineNumber >= s.startLine && range.endLineNumber <= s.endLine)
            .map(suggestion => ({
                ...
                edits: [{
                  ...
                  edit: {
                    range: new monaco.Range(
                      ...
                      suggestion.endLine!, model.getLineMaxColumn(suggestion.endLine!)
                    ),
    ...
    // AI feature handlers
    const handleGenerate = React.useCallback(async (prompt: string) => {
      ...
      try {
        ...
        if (multiFileSupport && currentActiveFile) {
          setCurrentFiles(prev => prev.map(f =>
            f.name === currentActiveFile ? { ...f, content: generatedCode } : f
          ))
        }
    ...
    const handleExplain = React.useCallback(async (selectedLine?: number) => {
      if (!onExplain) return
    ...
    const handleFeatureAction = React.useCallback(async (feature: CodeFeature) => {
    ...
    }, [isProcessing, editorValue, onRefactor, onDebug, onTest, onDocument, onReview, onChange])
    ...
    // File management
    const handleFileChange = React.useCallback((fileName: string) => {
    ...
    const handleFileDelete = React.useCallback((fileName: string) => {
        ...
        >
          ...
          <div className="flex items-center justify-between border-b bg-muted/50 p-2">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {showSidePanel ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />}
              </Button>
          ...
          {multiFileSupport && (
            <FileTabs
              ...
              onFileDelete={currentFiles.length > 1 ? handleFileDelete : undefined}
            />
          )}
            ...
            {showSidePanel && (
              <>
                ...
                <ResizablePanel defaultSize={30} minSize={20} maxSize={50}>
                  <Tabs value={sidePanelTab} onValueChange={(value) => setSidePanelTab(value as any)}>
                    <TabsList className="grid w-full grid-cols-3">
                      ...
                      <TabsTrigger value="security" className="text-xs">
                        ...
                        {securityIssues.length > 0 && (
                          <Badge
                            variant={securityIssues.some(i => i.severity === 'critical') ? 'destructive' : 'secondary'}
                    ...
                    <TabsContent value="suggestions" className="h-[calc(100%-40px)] overflow-hidden">
                    ...
                    <TabsContent value="security" className="h-[calc(100%-40px)] overflow-hidden">
                      <ScrollArea className="h-full p-3">
                        <SecurityPanel
