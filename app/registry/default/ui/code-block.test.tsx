...
// Mock shiki
jest.mock('shiki', () => ({
  codeToHtml: jest.fn().mockResolvedValue('<pre><code>const test = "hello"</code></pre>')
}))
...
describe('CodeBlock', () => {
  ...
  it('applies correct theme classes', () => {
    ...
    expect(screen.getByRole('group')).toHaveClass('bg-slate-50', 'border-slate-200')

    rerender(<CodeBlock {...defaultProps} theme="dracula" />)
    expect(screen.getByRole('group')).toHaveClass('bg-slate-900', 'border-purple-800/30')
  })
  ...
  it('applies custom max height', () => {
    ...
    const codeContainer = screen.getByRole('group').querySelector('[style*="max-height"]')
  ...
  it('handles highlighted lines correctly', async () => {
    ...
    await waitFor(() => {
      ...
      expect(lines[0].closest('div')).toHaveClass('bg-blue-500/10', 'border-l-blue-500')
    })
  })
