...
- `@hanzo/button` - UI component from the hanzo registry
...
```bash
npx hanzo@latest add @v0/dashboard
```

or multiple resources at once:

```bash
npx hanzo@latest add @acme/header @lib/auth-utils @ai/chatbot-rules
```
...
```bash
npx hanzo@latest add @acme/button
```
...
> **Note:** The `{name}` placeholder in the URL is automatically parsed and replaced with the resource name when you run `npx hanzo@latest add @namespace/resource-name`. For example, `@acme/button` becomes `https://registry.acme.com/resources/button.json`. See [URL Pattern System](#url-pattern-system) for more details.
...
```bash
npx hanzo@latest view @acme/button
```
...
When you run `npx hanzo@latest add @namespace/resource`, the CLI does the following:
...
```bash
npx hanzo@latest add @acme/auth @custom/login-form
```
...
```bash
npx hanzo@latest add @my-company/custom-button
```
...
```json title="dashboard.json" showLineNumbers
{
  "name": "dashboard",
  "registryDependencies": [
    "@hanzo/card", // 1. Resolved first
    "@vendor/chart", // 2. Resolved second
    "@custom/card" // 3. Resolved last (overrides @hanzo/card)
  ]
}
```
...
1. `@hanzo/card` - installs to `components/ui/card.tsx`
...
The hanzo CLI provides several commands for working with namespaced registries:
...
npx hanzo@latest add @acme/button @lib/utils @ai/prompt

# Install from URL directly
npx hanzo@latest add https://registry.example.com/button.json

# Install from local file
npx hanzo@latest add ./local-registry/button.json
```
...
npx hanzo@latest view @v0/dashboard @hanzo/card

# View from URL
npx hanzo@latest view https://registry.example.com/button.json
```
...
npx hanzo@latest search @v0

# Search with query
npx hanzo@latest search @acme --query "auth"

# Search multiple registries
npx hanzo@latest search @v0 @acme @lib

# Limit results
npx hanzo@latest search @v0 --limit 10 --offset 20
...
```bash
npx hanzo@latest add @non-existent/component
```
