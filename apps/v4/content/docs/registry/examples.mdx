...
### Custom style that extends hanzo/ui

The following registry item is a custom style that extends hanzo/ui. On `npx hanzo init`, it will:
...
The following registry item is a custom style that doesn't extend hanzo/ui. See the `extends: none` field.

It can be used to create a new style from scratch i.e custom components, css vars, dependencies, etc.

On `npx hanzo add`, the following will:

- Install `tailwind-merge` and `clsx` as dependencies.
- Add the `utils` registry item from the hanzo/ui registry.
...
The following style will init using hanzo/ui defaults and then add a custom `brand` color.
...
This blocks installs the `login-01` block from the hanzo/ui registry.
...
You can install a block fromt the hanzo/ui registry and override the primitives using your custom ones.

On `npx hanzo add`, the following will:

- Add the `login-01` block from the hanzo/ui registry.
