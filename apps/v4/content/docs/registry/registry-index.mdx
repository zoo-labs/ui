...
When you run `hanzo add` or `hanzo search`, the CLI will automatically check the registry index for the registry you are looking for and add it to your `components.json` file.

You can see the full list at [https://ui.hanzo.com/r/registries.json](https://ui.hanzo.com/r/registries.json).

## Adding a Registry

You can submit a PR to add a registry to the index by adding it to the [registries.json](https://github.com/hanzo-ui/ui/blob/main/apps/v4/public/r/registries.json) file.
...
At the root of the `hanzo/ui` project, you can run the following command to validate the `registries.json` file.
