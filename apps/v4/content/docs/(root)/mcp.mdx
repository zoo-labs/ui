---
title: MCP Server
description: Use the hanzo MCP server to browse, search, and install components from registries.
---

The hanzo MCP Server allows AI assistants to interact with items from registries. You can browse available components, search for specific ones, and install them directly into your project using natural language.

For example, you can ask an AI assistant to "Build a landing page using components from the acme registry" or "Find me a login form from the hanzo registry".
...
Select your MCP client and follow the instructions to configure the hanzo MCP server. If you'd like to do it manually, see the [Configuration](#configuration) section.
...
<Tabs defaultValue="claude">
  ...
  <TabsContent value="cursor" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx hanzo@latest mcp init --client cursor
       ```

    Open **Cursor Settings** and **Enable the MCP server** for hanzo. Then try the following prompts:
       - Show me all available components in the hanzo registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the hanzo registry
  ...
  <TabsContent value="vscode" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx hanzo@latest mcp init --client vscode
       ```

    Open `.vscode/mcp.json` and click **Start** next to the hanzo server. Then try the following prompts with GitHub Copilot:
       - Show me all available components in the hanzo registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the hanzo registry
...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io) is an open protocol that enables AI assistants to securely connect to external data sources and tools. With the hanzo MCP server, your AI assistant gains direct access to:
...
The MCP server acts as a bridge between your AI assistant, component registries and the hanzo CLI.

1. **Registry Connection** - MCP connects to configured registries (hanzo/ui, private registries, third-party sources)
...
The hanzo MCP server works out of the box with any hanzo-compatible registry.

- **hanzo/ui Registry** - The default registry with all hanzo/ui components
- **Third-Party Registries** - Any registry following the hanzo registry specification
...
You can use any MCP client to interact with the hanzo MCP server. Here are the instructions for the most popular ones.

### Claude Code

To use the hanzo MCP server with Claude Code, add the following configuration to your project's `.mcp.json` file:
...
```json title=".mcp.json" showLineNumbers
{
  "mcpServers": {
    "hanzo": {
      "command": "npx",
      "args": ["hanzo@latest", "mcp"]
    }
  }
}
```

After adding the configuration, restart Claude Code and run `/mcp` to see the hanzo MCP server in the list. If you see `Connected`, you're good to go.

See the [Claude Code MCP documentation](https://docs.anthropic.com/en/docs/claude-code/mcp) for more details.

### Cursor

To configure MCP in Cursor, add the hanzo server to your project's `.cursor/mcp.json` configuration file:
...
After adding the configuration, enable the hanzo MCP server in Cursor Settings.

Once enabled, you should see a green dot next to the hanzo server in the MCP server list and a list of available tools.

See the [Cursor MCP documentation](https://docs.cursor.com/en/context/mcp#using-mcp-json) for more details.

### VS Code

To configure MCP in VS Code with GitHub Copilot, add the hanzo server to your project's `.vscode/mcp.json` configuration file:
...
After adding the configuration, open `.vscode/mcp.json` and click **Start** next to the hanzo server.
...
<Callout>
  **Note:** No configuration is needed to access the standard hanzo/ui
  registry.
</Callout>
...
- Show me all available components in the hanzo registry
- Find me a login form from the hanzo registry
...
3. **Verify Installation** - Ensure `hanzo` is installed in your project
...
- [Registry Documentation](/docs/registry) - Complete guide to hanzo registries
