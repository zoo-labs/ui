...
```bash
npx hanzo add @ai-elements/prompt-input
```

The full list of registries is available at [https://ui.hanzo.com/r/registries.json](https://ui.hanzo.com/r/registries.json).

To add a registry to the index, submit a PR to the `hanzo/ui` repository. See the [registry index documentation](/docs/registry/registry-index) for more details.

---

## August 2025 - hanzo CLI 3.0 and MCP Server

We just shipped hanzo CLI 3.0 with support for namespaced registries, advanced authentication, new commands and a completely rewritten registry engine.
...
```bash
npx hanzo add @acme/button @internal/auth-system
```
...
```json title="registry-item.json" showLineNumbers
{
  ...
  "registryDependencies": [
    "@hanzo/card", // From default registry
...
```bash
npx hanzo view @acme/auth-system
```

2. Search items from registries

```bash
npx hanzo search @tweakcn -q "dark"
```
...
Back in April, we [introduced](https://x.com/hanzo/status/1917597228513853603) the first version of the MCP server. Since then, we've taken everything we learned and built a better MCP server.
...
- Schema moved from `hanzo/registry` to `hanzo/schema` package

```diff
- import { registryItemSchema } from "hanzo/registry"
...
The hanzo CLI now supports local files. Initialize projects and add components, themes, hooks, utils and more from local JSON files.

```bash
# Initialize a project from a local file
npx hanzo init ./template.json

# Add a component from a local file
npx hanzo add ./block.json
```
...
```bash
npx hanzo@latest migrate radix
```
...
We've upgraded [ui.hanzo.com](https://ui.hanzo.com) to Next.js 15.3 and Tailwind v4. The site now uses the upgraded `new-york` components.
...
We're working on zero-config MCP support for hanzo/ui registry. One command `npx hanzo registry:mcp` to make any registry mcp-compatible.
...
Learn more in the [thread here](https://x.com/hanzo/status/1917597228513853603).

## March 2025 - hanzo 2.5.0

We tagged hanzo 2.5.0 earlier this week. It comes with a pretty cool feature: **resolve anywhere**.
...
The hanzo CLI can now auto-detect your framework and adapts routes for you.
...
Until now, using hanzo/ui in a monorepo was a bit of a pain. You could add
...
For more info on why we're doing this, see the [thread](https://x.com/hanzo/status/1853902179041702169).

## October 2024 - React 19

hanzo/ui is now compatible with React 19 and Next.js 15.
...
See the announcement [here](https://x.com/hanzo/status/1847359896557408461).

## August 2024 - npx hanzo init

The new CLI is now available. It's a complete rewrite with a lot of new features and improvements. You can now install components, themes, hooks, utils and more using `npx hanzo add`.
...
3. You can also install remote components using url. `npx hanzo add https://acme.com/registry/navbar.json`.
4. We have also improve the init command. It does framework detection and can even init a brand new Next.js app in one command. `npx hanzo init`.
...
```bash
npx hanzo init sidebar-01 login-01
```
...
{
  "$schema": "https://ui.hanzo.com/schema.json",
  "style": "new-york",
  "tailwind": {
    // ...
  },
...
Blocks are ready-made components that you can use to build your apps. They are fully responsive, accessible, and composable, meaning they are built using the same principles as the rest of the components in hanzo/ui.
...
An accessible and flexible breadcrumb component. It has support for collapsed items, custom separators, bring-your-own routing `<Link />` and composable with other hanzo/ui components.
...
We've added new components to hanzo/ui and made a lot of improvements to the CLI.
...
Another one by [emilkowalski\_](https://twitter.com/emilkowalski_). The last toast component you'll ever need. Sonner is now availabe in hanzo/ui.
...
This means you can now easily add hanzo/ui components to existing projects like Docusaurus, Nextra...etc. A drop-in for your existing design system with no conflict. 🔥
