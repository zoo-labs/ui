...
If you're starting a new registry project, you can use the [registry template](https://github.com/hanzo-ui/registry-template) as a starting point. We have already configured it for you.

## registry.json

The `registry.json` file is only required if you're using the `hanzo` CLI to build your registry.
...
Note: the `build` command is currently only available in the `hanzo@canary` version of the CLI.

```bash
npm install hanzo@canary
```
...
You can change the output directory by passing the `--output` option. See the [hanzo build command](/docs/cli#build) for more information.
...
The `hanzo` CLI does not offer a built-in way to add auth to your registry. We recommend handling authorization on your registry server.
...
Use the secure token to authenticate requests and return a 401 Unauthorized response if the token is invalid. Both the `hanzo` CLI and `Open in v0` will handle the 401 response and display a message to the user.
...
To install a registry item using the `hanzo` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx hanzo@latest add http://localhost:3000/r/hello-world.json
```
